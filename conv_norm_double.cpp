...
#include<stdlib.h>
using namespace std;
void my2d_arrayPrint(double** arr,int n){
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            cout<<arr[i][j]<<" ";
        }
...
double unit_conv(double** layer,int row_start,int col_start,double** kernel,int kernel_size){
    double sum=0;
...
double** basic_conv_operation(double** layer,double** kernel,int layer_size,int kernel_size){
    int sub_row_start=0;
    int sub_col_start=0;
    int sub_start=0;
    int sub_end=layer_size-kernel_size+1;
    double** res_mat=create_matrix(layer_size-kernel_size+1);
...
void fill_matrix_user(double** arr,int n){
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
...
void fill_matrix_user_random(double** arr,int n){
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            //cin>>arr[i][j];
            arr[i][j]=rand()%10;
...
int main(){
    ...
    int kernel_size;
    cin>>kernel_size;
    double** layer_mat=create_matrix(layer_size);
    double** kernel_mat=create_matrix(kernel_size);
    ...
    double** res=basic_conv_operation(layer_mat,kernel_mat,layer_size,kernel_size);
